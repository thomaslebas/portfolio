(function () {
  ...
  var shaderCode = [
    ...
    '  let eps: f32 = 0.0015;',
    '  let hL = rippleHeight(uv - vec2<f32>(eps, 0.0));',
    '  let hR = rippleHeight(uv + vec2<f32>(eps, 0.0));',
    '  let hD = rippleHeight(uv - vec2<f32>(0.0, eps));',
    '  let hU = rippleHeight(uv + vec2<f32>(0.0, eps));',
    '',
    '  let gradX = (hR - hL) / (2.0 * eps);',
    '  let gradY = (hU - hD) / (2.0 * eps);',
    '',
    '  let dispUV = uv + vec2<f32>(gradX, gradY) * uniforms.amplitude;',
  ...
  function drawTextTexture() {
    ...
    computeWordRects().forEach(function (word) {
      textCtx.fillText(word.text, word.x, word.y);
    });

    return { width: width, height: height };
  }

  function createTextureFromCanvas(width, height) {
  ...
  function resize() {
    ...
    rippleCanvas.width = Math.round(dims.width * dpr);
    rippleCanvas.height = Math.round(dims.height * dpr);
    createTextureFromCanvas(dims.width, dims.height);
    createBindGroup();
  }
  ...
  function frame(now) {
    ...
    if (!running) {
      running = true;
      heading.appendChild(rippleCanvas);
      requestAnimationFrame(function () {
        heading.classList.add('has-ripple');
      });
    }
  ...
  function init() {
    navigator.gpu.requestAdapter().then(function (adapter) {
      ...
      gpuContext = rippleCanvas.getContext('webgpu');
      ...
      reduceMotionQuery.addEventListener('change', function (e) {
        if (e.matches) {
          destroyed = true;
          stopLoop();
          heading.classList.remove('has-ripple');
          if (rippleCanvas.parentNode) rippleCanvas.parentNode.removeChild(rippleCanvas);
        }
      });
    }).catch(function () {
